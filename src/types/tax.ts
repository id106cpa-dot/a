...
export interface TaxInputData {
  ...
  paysAlimony: boolean; // מזונות לבן/בת זוג לשעבר
  ...
  // Step 5: Academic Degree & Military Service
  ...
  // עולים חדשים ותושבים חוזרים (סעיף 35)
  isNewImmigrant: boolean;
  immigrationYear?: number;

  // Step 6: Settlement Tax Credit (יישוב מזכה סעיף 11)
  livesInEligibleSettlement: boolean;
  settlementName?: string;
  monthsInSettlement?: number;

  // Step 7: Capital Market (שוק ההון - טופס 867)
  investedInCapitalMarket: boolean;
  capitalGains: number; // רווחי הון ריאליים
  capitalLosses: number; // הפסדי הון שלא קוזזו
  capitalTaxPaid: number; // מס שנוכה במקור בבנק

  // Step 8: Pension, Life Insurance, Donations & Deductions
  ...
  // Step 9: Medical / Disability
  hasMedicalExemption: boolean; // נכות 90%+ סעיף 9(5)
...
export interface CalculationResult {
  ...
  taxableIncome: number; // הכנסה חייבת במס לאחר ניכויים
